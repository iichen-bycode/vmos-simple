package com.vlite.app.dialog.bottomsheet;
...
//        nestedScrollingChildRef = new WeakReference<>(findScrollingChild(child));
        mNestedScrollingChildRefList = findNestedScrollingEnabledViews(child);
        return true;
    }

    public List<View> findNestedScrollingEnabledViews(View v) {
        // ViewCompat.isNestedScrollingEnabled(view)
        List<View> nestedScrollingEnabledViews = new ArrayList<>();
        ...
        while (!viewQueue.isEmpty()) {
            ...
            if (ViewCompat.isNestedScrollingEnabled(view)) {
                nestedScrollingEnabledViews.add(view);
            } else {
                if (view instanceof ViewGroup) {
                    final int childCount = ((ViewGroup) view).getChildCount();
                    for (int i = 0; i < childCount; i++) {
                        viewQueue.offer(((ViewGroup) view).getChildAt(i));
                    }
                }
            }
        }
        return nestedScrollingEnabledViews;
    }

    public void setNestedScrollingChildRefList(List<View> nestedScrollingChildRefList) {
...
//                && !parent.isPointInChildBounds(scroll, (int) event.getX(), (int) event.getY())
    ...
    private boolean isOneOfChild(View childView) {
        if (mNestedScrollingChildRefList != null) {
            for (View child : mNestedScrollingChildRefList) {
    ...
    public void onStopNestedScroll(
        ...
        if (!isOneOfChild(target) || !nestedScrolled) {
            return;
        }
        int top;
        int targetState;
            ...
            new ViewDragHelper.Callback() {
                ...
                public boolean tryCaptureView(@NonNull View child, int pointerId) {
                    ...
                    if (state == STATE_EXPANDED && activePointerId == pointerId) {
                        ...
                        if (mNestedScrollingChildRefList != null) {
                            for (View childView : mNestedScrollingChildRefList) {
                                if (childView != null && ViewCompat.canScrollVertically(childView, -1)) {
                                    // Let the content scroll up
                                    return false;
                                }
                            }
