...
public class LauncherAdaptiveIconDrawable extends AdaptiveIconDrawable implements Drawable.Callback {
    ...
    private static final String sMaskPath = "M0,20 q0,-20 20,-20 L80,0 q20,0 20,20 L100,80 q0,20 -20,20 L20,100 q-20,0 -20,-20 Z";
    ...
    /**
     * Indices used to access {#mLayerState.mChildDrawable} array for foreground and
     * background layer.
     */
...
//            Log.e("AdaptiveIconDrawable", "shape not initialized", new Throwable());
    ...
    public static @NonNull Drawable wrapNonNull(@NonNull Drawable icon) {
        if (icon.getClass() == AdaptiveIconDrawable.class) {
            return new LauncherAdaptiveIconDrawable((AdaptiveIconDrawable) icon);
        }
        return icon;
    }
     ...
     */
    public LauncherAdaptiveIconDrawable(Drawable backgroundDrawable,
    ...
    public LauncherAdaptiveIconDrawable(AdaptiveIconDrawable drawable) {
    ...
    public void clearMutated() {
        final Class<? extends LauncherAdaptiveIconDrawable> cls = getClass();
        try {
            // super.clearMutated();
            Method clearMutatedMethod = cls.getSuperclass().getDeclaredMethod("clearMutated");
            clearMutatedMethod.setAccessible(true);
            clearMutatedMethod.invoke(this);
        } catch (Exception e) {
            e.printStackTrace();
        }
        final ChildDrawable[] array = mLayerState.mChildren;
        for (int i = 0; i < mLayerState.N_CHILDREN; i++) {
            ...
            if (dr != null) {
                try {
                    // dr.clearMutated();
                    Method clearMutatedMethod = cls.getDeclaredMethod("clearMutated");
                    clearMutatedMethod.setAccessible(true);
                    clearMutatedMethod.invoke(this);
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    static class LayerState extends ConstantState {
        ...
        public Drawable newDrawable() {
            return new LauncherAdaptiveIconDrawable(this, null);
        }

        @Override
        public Drawable newDrawable(@Nullable Resources res) {
            return new LauncherAdaptiveIconDrawable(this, res);
        }
