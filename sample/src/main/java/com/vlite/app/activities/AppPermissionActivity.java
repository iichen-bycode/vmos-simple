...
import com.vlite.app.adapters.PermissionAdapter;
import com.vlite.app.bean.AppPermissionInfo;
import com.vlite.app.databinding.ActivityAppPermissionBinding;
    ...
    private void loadAppPermissions(String packageName) {
        new AsyncTask<Void, Void, List<AppPermissionInfo>>() {
            ...
            protected List<AppPermissionInfo> doInBackground(Void... voids) {
                ...
                final String[] permissions = VLite.get().getDangerousPermissions(packageName);
                final int[] permissionResults = VLite.get().checkPermissions(packageName, permissions);
    ...
    private void bindDataForViews(List<AppPermissionInfo> result) {
        ...
        mPermissionAdapter.setOnItemClickListener((view, position) -> {
            ...
            if (PackageManager.PERMISSION_GRANTED == item.getPermissionResult()) {
                // 关闭权限
                VLite.get().setPermissionResult(mPackageName, item.getPermission(), PackageManager.PERMISSION_DENIED);
